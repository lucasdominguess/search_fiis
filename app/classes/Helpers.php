...
use App\uteis_php\Classes\CreateLogger;
...
class Helpers{
    ...
    return match ($date) {
        'Fri' => [$GLOBALS['date'], 'Mon'],// forçar data para segunda feira
        // 'Wed' => [$GLOBALS['DAY']->modify('-1 day')->format('Y-m-d'), 'Tue'],
    ...
    public function monitoraFormat($dados) {
        ...
        foreach ($dados as &$linha) {
            unset($linha['id'], $linha['id_unidade'],$linha['id_adm']);
            $date = new \Datetime($linha['data']);
            $linha['data'] = $date->format('d/m/Y');
        }
        return $dados;
    }
    public function Sqlgenerate()
    {   
       $log = $this->logGenerate();
        return $sqlGen = new  Sql($log);
    }
    public function verifyRegexArray(string |array $dados,$regex){

        foreach ($dados as $key => $value) {
            if(!preg_match($regex,$value)){
                throw new \Exception("Dados enviados em $key não correspondem ao padrão requerido");
            }
